"""
tests/performance/locustfile.py — Load tests with Locust.

═══════════════════════════════════════════════════════════════════════
WHAT ARE PERFORMANCE TESTS?
═══════════════════════════════════════════════════════════════════════
Performance tests verify that the application behaves correctly under
LOAD — when many users are using it simultaneously.

They answer questions like:
  - Can we handle 50 concurrent users without errors?
  - What is the average / 95th-percentile response time under load?
  - Does a missing database index cause degradation? (Bug 4 scenario)
  - Which endpoint is the bottleneck?

WHY LOCUST?
  - Load tests are written in plain Python — easy for beginners to read
  - Scales from a laptop to hundreds of workers
  - Real-time web UI at http://localhost:8089

WHAT IT CATCHES THAT SMOKE TESTS MISS:
  Smoke tests make ONE request at a time against a nearly empty database.
  Locust simulates 50 concurrent users against a populated database.

  Example: a missing index on a frequently-queried column causes a full
  table scan on every request. With 1 user and 10 rows → imperceptibly fast.
  With 50 concurrent users and 10,000 rows → response times explode.

HOW TO RUN:
  # Web UI (recommended for live demos):
  docker compose run --rm tests locust \\
